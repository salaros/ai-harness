...
The npm, pnpm and yarn rows list `.astro`, and their scaffolds both install `prettier-plugin-astro` and name it in the project's prettier config. The extension and its plugin travel together, asserted both ways in `.agents/hooks/tests/tables/stacks.js`: a row listing the extension installs and declares the plugin, and a row installing the plugin lists the extension.
...
`.razor` stays off every list. It has no prettier plugin, and `dotnet format` is no substitute: it leaves a misformatted `.razor` file byte-identical while reformatting the same `@code` body written into a `.cs` file. Nothing on hand can check it, so nothing claims to.
...
- A project the harness scaffolds can check `.astro` from the start. One that predates the scaffold cannot: its first push touching an `.astro` file fails until it installs the plugin and names it in its own prettier config. That is the loud failure D-1 chose, arriving where somebody can act on it.
