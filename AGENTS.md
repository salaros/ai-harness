...
- After opening a pull request, or when the session brief lists open ones, offer to run `pr-sweep` on them; the skill says how to ask and what a yes sets in motion. Its merges need `gh pr merge` allowed in the harness's permissions, which `project-init` offers to set; without that the sweep stops before each merge and hands it to the user.
- End a session, after the last push, with `duck-debt`, then offer the user `/retro`, which only they can start. The session brief names whichever of the two this repo installed.
