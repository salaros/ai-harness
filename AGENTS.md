...
- After opening a pull request, or when the session brief lists open pull requests nobody has offered to sweep, ask through the question tool whether to run `pr-sweep` on them. It reviews each, fixes the findings and unresolved comments, brings it up to date with its base and merges it once its checks pass. It never starts without a yes.
