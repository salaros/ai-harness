...
- **Domain docs**: `CONTEXT.md` holds the glossary and `docs/adr/` the decisions, both created lazily by `domain-modeling`; use their terms when they exist. A `microservices` repo splits them per service instead, as `docs/agents/domain.md` describes.
...
- Write commit messages as conventional commits with a body, in English: `docs/agents/writing.md` has the rules the `commit-msg` hook enforces, along with which files follow `writing-clearly-and-concisely`, which follow `writing-for-agents`, and what `MEMORY.md`'s `Prose language` governs.
