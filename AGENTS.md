...
- After opening a pull request, or when the session brief lists open ones, offer to run `pr-sweep` on them; the skill says how to ask and what a yes sets in motion. Its merges need `gh pr merge` allowed, which `project-init` offers to set up.
- End a session, after the last push, as the "end of a session" row in `.agents/routing.md` says; the session brief names the skills from it this repo installed.
