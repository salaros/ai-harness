...
| --- | --- | --- |
| `session-start.js` | session start | Prints the branch, whether Git hooks are installed, skills missing from disk, and whether `CONTEXT.md`, `docs/adr/` and the issue-tracker config exist. On a GitHub clone with `gh` signed in, it also lists the open pull requests by number, so the agent can offer `pr-sweep`. Last, how to end the session: `duck-debt`, then an offer of `/retro`, whichever of the two are installed. |
