...
6. **Allow the merge.** Ask, with the question tool, whether agents may merge the pull requests they sweep: `this clone only` (recommended, first), `everyone`, `no`. `pr-sweep` merges with `gh pr merge`, which a harness may refuse an agent until a permission rule allows it. On anything but `no`, add the rule to Claude Code's allow list, keeping what the list already holds: `"permissions": { "allow": ["Bash(gh pr merge:*)"] }`, in `.claude/settings.local.json` for `this clone only` or `.claude/settings.json` for `everyone`. Claude Code asks the user before any write to those files, so expect the prompt. OpenCode runs bash unless `opencode.json` sets `permission.bash` to `ask` or `deny`; only then, and only for `everyone`, add `"gh pr merge *": "allow"` beside it. For a harness with no permission file, say that the sweep will stop at each merge and hand it back. Allow nothing wider than the merge: `git push` and the read-only `gh` calls already run, and a blanket `Bash(gh:*)` would also let an agent close issues or edit repository settings. Done when each wired harness has the rule, needs none, or the report says why not.
